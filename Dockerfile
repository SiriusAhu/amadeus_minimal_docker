...
# 4. 配置 rosdep 并使用清华大学镜像源更新（同时安装pyserial库）
RUN pip install -i https://pypi.tuna.tsinghua.edu.cn/simple rosdepc pyserial && \
    rosdepc init && \
    rosdepc update
...
# 10. 设置容器启动命令 (Entrypoint)
COPY <<'EOF' /ros_entrypoint.sh
#!/bin/bash
set -e
source "/opt/ros/humble/setup.bash"
source "/ros2_ws/install/setup.bash"
exec ros2 launch minimal_bringup minimal_motion.launch.py
EOF
